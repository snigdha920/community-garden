/* eslint-disable @next/next/no-img-element */
import React from 'react';
import { TextIcon } from './TextIcon';

export const AboutTheProject: React.FC = () => {
  const pageData: Array<{ paragraph: string; icon: React.ReactElement }> = [
    {
      paragraph:
        'Improving food security in local communities and access to healthy, locally grown foods are major goals of the Community Gardens movement. The community gardens movement provides neighborhoods and communities with access to local spaces that can be cultivated for growth of communally grown fruits and vegetables that can provide food and support to members of that community.',
      icon: <img src="/icons/vegetables.png" alt="food-safety-icon" width="80px" />,
    },
    {
      paragraph:
        'In keeping with its mission to serve as a key resource to the campus and its local community partners, the Learning & Research Technologies group in the Division of IT&IRat Cal Poly Pomona has begun the Community Gardens Technology Support Initiative. The goal is to provide community gardens with data that will assist in informed decision-making  and increased crop yields.',
      icon: <img src="/icons/teamwork.svg" alt="teamwork-icon" width="80px" />,
    },
    {
      paragraph:
        'LR&T will provide sensors that collect data such as soil moisture, temperature, and acidity; air temperature; wind speed; and more. These data will be monitored, queried, and made into reports and alerts that will inform the garden',
      icon: <img src="/icons/bar-chart.svg" alt="bar-chart-icon" width="80px" />,
    },
  ];
  return (
    <div>
      {pageData.map((paragraph, index) => {
        return (
          <div key={index}>
            <TextIcon icon={paragraph.icon}>{paragraph.paragraph}</TextIcon>
          </div>
        );
      })}
    </div>
  );
};
